...
function renderNationsList() {
  ...
  if (searchKeyword) {
    ...
    filteredNations = filteredNations.filter(nation =>
      nation.name.toLowerCase().includes(keyword) ||
      nation.pinyin.toLowerCase().includes(keyword) ||
      nation.tags.some(tag => tag.toLowerCase().includes(keyword))
    );
  }
  ...
  listContainer.innerHTML = filteredNations.map(nation => `
    <div class="nation-card" onclick="goToNationDetail('${nation.id}')">
      <div class="nation-card-image" ${nation.image ? `style="background: url('${nation.image}') no-repeat center center; background-size: cover;"` : ''}>
        ${nation.image ? '' : nation.name.charAt(0)}
      </div>
